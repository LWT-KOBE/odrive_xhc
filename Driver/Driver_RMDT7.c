...
#include "Util.h"
/******************************外部调用函数************************************/
static void Driver_RMDT7_Init(USART_TypeDef* USARTx,BSP_GPIOSource_TypeDef *USART_RX,uint8_t PreemptionPriority,uint8_t SubPriority);
void Driver_RMDT7_Decode_RemoteData(dt7RcSturct_t *RC_Ctrl,uint8_t *Array_Dbus);
static void DT7_Init(void);
/*****************************************************************************/
deviceInitClass DT7IintClass = {
	DT7_Init,
};
/*
***************************************************
...
static void Driver_RMDT7_Init(USART_TypeDef* USARTx,BSP_GPIOSource_TypeDef *USART_RX,uint8_t PreemptionPriority,uint8_t SubPriority){
	...
	//波特率为100000
	DBUS_USART.USART_InitStructure.USART_BaudRate = 100000;									
	//字长为8位数据格式
	DBUS_USART.USART_InitStructure.USART_WordLength = USART_WordLength_8b;	
	//一个停止位
	DBUS_USART.USART_InitStructure.USART_StopBits = USART_StopBits_1;				
	//偶校验位
	DBUS_USART.USART_InitStructure.USART_Parity = USART_Parity_Even;				
	//接收模式
	DBUS_USART.USART_InitStructure.USART_Mode = USART_Mode_Rx;								
	//无硬件数据流控制
	DBUS_USART.USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None,	
