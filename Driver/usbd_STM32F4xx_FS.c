...
void USBD_Init (void) {
  ...
  OTG->GCCFG      &= ~(3 << 18);        /* No VBUS sensing device "B"         */
	OTG->GCCFG      |= (1 << 21);         /* No VBUS sensing device "B"         */
...
void USBD_Connect (uint32_t con) {
  if (con) {
    OTG->GCCFG  |=  (3 << 18) |         /* enable VBUS sensing device "B"     */
                    (1 << 16);          /* power down deactivated             */
    OTG->DCTL   &= ~(1 <<  1);          /* soft disconnect disabled           */
  }
  else {
    OTG->DCTL   |=  (1 <<  1);          /* soft disconnect enabled            */
    OTG->GCCFG  &= ~(3 << 18) |         /* disable VBUS sensing device "B"    */
