...
void RMDLMotorPIDctrl(uint8_t cmd,CAN_TypeDef *CANx,uint32_t ID_CAN,RMDLCANSendStruct_t* CanSendData) {
	...
	CanSendData->data[1] = (uint8_t)pidCommit.angle.KP;
	CanSendData->data[2] = (uint8_t)pidCommit.angle.KI;
	
	CanSendData->data[3] = (uint8_t)pidCommit.rate.KP;
	CanSendData->data[4] = (uint8_t)pidCommit.rate.KI;
	CanSendData->data[5] = (uint8_t)pidCommit.iq.KP;
	CanSendData->data[6] = (uint8_t)pidCommit.iq.KI;
	RMDLMolotovSendData(CANx,ID_CAN,CanSendData);
}
/*加速度类*/
