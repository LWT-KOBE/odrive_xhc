...
void dmaNvicConfig(uint8_t irqN,uint8_t PreemptionPriority,uint8_t SubPriority)
{
...
void SK6812Config(void){
  ...
  BIT_COMPARE_0 = (uint16_t)typePeriod / 11 * 3;
	BSP_TIM_PWM_Init(TIM4,typePeriod,typePrescaler,BSP_GPIOB6,NULL,NULL,NULL);	
	dmaNvicConfig(DMA1_Stream0_IRQn,1,0);
	BSP_DMA_SK6812_Init(&BSP_DMA_TIM4_CH1,(uint32_t)&TIM4->CCR1,(uint32_t)sk6812Data.ledStripDMABuffer,SK6812_DMA_BUFFER_SIZE);
}
...
void SK6812_SendData(void){		
	DMA_SetCurrDataCounter(BSP_DMA_TIM4_CH1.DMA_Streamx,SK6812_DMA_BUFFER_SIZE);	
	TIM_SetCounter(TIM4, 0);				
	TIM_DMACmd(TIM4,TIM_DMA_CC1, ENABLE);
	DMA_Cmd(BSP_DMA_TIM4_CH1.DMA_Streamx, ENABLE);
}
...
void DMA1_Stream0_IRQHandler(void){																	//DMA1-0
	if(DMA_GetITStatus(DMA1_Stream0,DMA_IT_TCIF0)){
		DMA_Cmd(DMA1_Stream0, DISABLE);																	//失能
		TIM_DMACmd(TIM4,TIM_DMA_CC1, DISABLE);													//取消dma更新tim比较
		DMA_ClearITPendingBit(DMA1_Stream0, DMA_IT_TCIF0);							//清除IT_FLAG
	}
}
