...
u16	 BSP_SPI_ReadWriteByte(BSP_SPI_TypeDef* BSP_SPIx,u16 TxData);
/*****************************************************************************/
...
u16 BSP_SPI_ReadWriteByte(BSP_SPI_TypeDef* BSP_SPIx,u16 TxData){
	u16 retry=0;		
	while(SPI_I2S_GetFlagStatus(BSP_SPIx->SPIx, SPI_I2S_FLAG_TXE) == RESET){	//等待发送区空 
		retry++;
		if(retry>1000)	
			return 0;
	}	
	...
	retry=0;
	while(SPI_I2S_GetFlagStatus(BSP_SPIx->SPIx, SPI_I2S_FLAG_RXNE) == RESET){	//等待接收完一个byte
		retry++;
		if(retry>1000)	
			return 0;
	}
