...
void BSP_TIM_ICAP_Init(TIM_TypeDef* BSP_TIMx,uint32_t Period,u16 Prescaler,
	...
	if(ICAP1 != NULL){
		...
		GPIO_PinAFConfig(ICAP1->ICAP_GPIO->GPIOx,GPIO_Pin_TO_GPIO_PinSource(ICAP1->ICAP_GPIO),TIM_TO_GPIO_AF(BSP_TIMx));	//ICAP1引脚复用为定时器TIMx
	}
	if(ICAP2 != NULL){
		BSP_GPIO_Init(ICAP2->ICAP_GPIO,GPIO_Mode_AF_PP);	//配置ICAP2引脚为复用推挽模式
		GPIO_PinAFConfig(ICAP2->ICAP_GPIO->GPIOx,GPIO_Pin_TO_GPIO_PinSource(ICAP2->ICAP_GPIO),TIM_TO_GPIO_AF(BSP_TIMx));	//ICAP2引脚复用为定时器TIMx
	}
	if(ICAP3 != NULL){
		BSP_GPIO_Init(ICAP1->ICAP_GPIO,GPIO_Mode_AF_PP);	//配置ICAP3引脚为复用推挽模式
		GPIO_PinAFConfig(ICAP3->ICAP_GPIO->GPIOx,GPIO_Pin_TO_GPIO_PinSource(ICAP3->ICAP_GPIO),TIM_TO_GPIO_AF(BSP_TIMx));	//ICAP3引脚复用为定时器TIMx
	}
	if(ICAP4 != NULL){
		BSP_GPIO_Init(ICAP4->ICAP_GPIO,GPIO_Mode_AF_PP);	//配置ICAP4引脚为复用推挽模式
		GPIO_PinAFConfig(ICAP4->ICAP_GPIO->GPIOx,GPIO_Pin_TO_GPIO_PinSource(ICAP4->ICAP_GPIO),TIM_TO_GPIO_AF(BSP_TIMx));	//ICAP4引脚复用为定时器TIMx
	}
